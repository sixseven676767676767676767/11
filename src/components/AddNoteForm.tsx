    ...
    youtube: [
      {
        ...
        text: "各位同學好，今天我們要深入探討愛因斯坦廣義相對論的核心預測：黑洞。廣義相對論在1915年提出，徹底顛覆了我們對牛頓萬有引力的認知。愛因斯坦指出，重力並非物體間的超距拉力，而是質量與能量造成『時空幾何的彎曲』。當一個巨大恆星燃料耗盡，在自身重力下崩塌，若其質量超越歐本海默極限，就會形成黑洞。黑洞的邊界稱為『事件視界』，在此邊界內，逃逸速度大於光速，因此連光也無法逃脫。在黑洞最深處存在一個體積無限小、密度無限大的『奇異點』，在此處現有的物理定律全面失效。觀測黑洞並非直接看見它，而是透過『重力透鏡效應』與吸積盤發出的強烈X射線。黑洞不僅是天文物理的極致天體，更是量子力學與廣義相對論交會的終極實驗室。"
      },
      {
        label: "⚙️ CPU 與二進位運算",
        title: "計算機科學基礎：CPU 的運算核心與二進位系統",
        url: "https://www.youtube.com/watch?v=fWqiiG2v9gQ",
        text: "各位同學們好，今天我們這節計算機概論要來討論：中央處理器 CPU 到底是怎麼編譯與執行程式碼的。我們在電腦上看到的各種多媒體、文字、網頁，在底層硬體晶片眼中全部都只是一連串極其龐大的「0 與 1」二進位訊號。二進位是以 2 爲底數的記數系統，其物理上對應到電晶體的「導通與截止」狀態。CPU 的核心包含控制單元（CU）與算術邏輯單元（ALU）。ALU 負責執行基本的二進位加減乘除，以及 AND、OR、NOT 等邏輯閘運算。CPU 的運作遵循預取（Fetch）、解碼（Decode）、執行（Execute）以及寫回（Writeback）的核心指令週期。微小的電晶體組成邏輯閘，邏輯閘又搭建出加法器與寄存器，最後組件與時鐘脈衝同步，完成了每秒數十億次的震撼運算，奠定了當代數位科技的全部基石。"
      },
      {
        label: "📈 經濟學供需法則",
        title: "經濟學原理：名校課堂供需法則與市場均衡分析",
        url: "https://www.youtube.com/watch?v=W3a__2mP1gI",
        text: "同學們好，今天我們探討經濟學的核心：供需法則與市場均衡。首先，『需求法則』指出在其他條件不變下，商品價格與需求量呈反向變動；當價格上升，消費者購買意願降低。相反地，『供給法則』顯示價格與供給量呈同向變動，價格愈高，廠商越有誘因生產。當市場上的需求曲線與供給曲線相交，即達到『市場均衡』。此時的交叉點決定了『均衡價格』與『均衡數量』。若市場價格高於均衡價格，會產生『超額供給』（即供過於求），促使價格下跌；若低於均衡價格，則產生『超額需求』（即供不應求），推動價格上漲。這種透過價格自動調節供需、消除短缺或賸餘的機制，就是亞當斯密所稱的『看不見的手』。此外，我們必須區分『需求量的變動』（因價格變動在同一曲線上移動）與『需求的變動』（因所得、偏好或替代品價格等外部因素導致整條曲線平移），這在分析市場衝擊時至關重要。"
      }
    ],
    audio: [
      {
